		...
		if ( (is_single() || is_page()) ){
			...
			if(!empty($post_parts)){
				...
				foreach($post_parts as $content){ $page++;
					...
					$slidecount = '<span class="jps-slide-count">'.$page.' of '.$numpages.'</span>';
					if($page == $numpages){
					...
					if((get_option( 'jpps_nav_position' ) == 'top')||(get_option( 'jpps_nav_position' ) == 'both')){
						$ppscontent .= '<nav class="jps-slider-nav jps-clearfix">';
					...
					// Slide Content
					//$ppscontent .= '<div class="jps-content jps-clearfix">'.$content.'</div>';
					$ppscontent .= '<div class="jps-content jps-clearfix">'.wpautop($content).'</div>';
			
					// Bottom Slider Navigation
					if((get_option( 'jpps_nav_position' ) == 'bottom')||(get_option( 'jpps_nav_position' ) == 'both')){
						$ppscontent .= '<nav class="jps-slider-nav jps-bottom-nav jps-clearfix">';
						$ppscontent .= wp_link_pages();
